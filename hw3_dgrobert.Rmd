...
#remove missing values for Boot function to work later
d=na.omit(d)
#log transform
...
# d is data source, model lets you use functions like lm() or others, reps is number of bootstrapped samples
  original.est <- get(model)(d)
# original est initial estimate to be replicated  
  n_est <- length(original.est)
#n_est calculates the number of outputs to be estimated
  temp <- matrix(NA, ncol = n_est , nrow = reps)
# temp is the empty matrix to hold bootstrapped values
  nobs <- nrow(d)
#nobs is total observations to resample from
#here is the actual bootstrapping process:  
  for (i in 1:reps) {
    posdraws <- ceiling(runif(nobs)*nobs)
    draw <- d[posdraws,]
    temp[i,] <- get(model)(draw)
  }
#  now we add a couple of add'l outputs including sd and CI
  sds <- apply(temp,2,sd)
